...
def fetch_recommendations(event):
    ...
    user_recom_hist = sqlite3.connect('/mnt/dbs/user_recom_hist.db')
    ...
    rcp_sims_db = sqlite3.connect('/mnt/dbs/rcps_sims.db')
    ...
    recoms = []
    if last_recoms == "None":
        recoms += random.sample(range(rcp_counts), n)
    else:
...
def id2rcp(ids):
    ...
    :return: dict/ json.
     Keys are 'rcp_names'
        ...
        'n_ingredients'
    """
    raw_rcps_db = sqlite3.connect('/mnt/dbs/raw_rcps.db')
...
def handler(event, context):
    # print(event)
    recom_ids = fetch_recommendations(event)
    recom_infos = id2rcp(recom_ids)

    # TODO implement
    return {
        'statusCode': 200,
        'body': json.dumps(recom_infos)
    }
